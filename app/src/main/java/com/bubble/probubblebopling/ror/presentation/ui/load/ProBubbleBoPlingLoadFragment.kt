...
import com.bubble.probubblebopling.databinding.FragmentLoadChickenBinding
...
class ProBubbleBoPlingLoadFragment : androidx.fragment.app.Fragment(R.layout.fragment_load_chicken) {
    private lateinit var chickenLoadBinding: FragmentLoadChickenBinding

    private val proBubbleBoPlingLoadViewModel by viewModel<ProBubbleBoPlingLoadViewModel>()

    private val chickenSharedPreference by inject<com.bubble.probubblebopling.ror.data.shar.ProBubbleBoPlingSharedPreference>()
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        lifecycleScope.launch {
            viewLifecycleOwner.repeatOnLifecycle(Lifecycle.State.STARTED) {
                proBubbleBoPlingLoadViewModel.chickenHomeScreenState.collect {
                    when (it) {
                        is ProBubbleBoPlingLoadViewModel.ChickenHomeScreenState.ChickenLoading -> {

                        }

                        is ProBubbleBoPlingLoadViewModel.ChickenHomeScreenState.ChickenError -> {
                            requireActivity().startActivity(
                                Intent(
                                    requireContext(),
                                    _root_ide_package_.com.bubble.probubblebopling.MainActivity::class.java
                                )
                            )
                            requireActivity().finish()
                        }

                        is ProBubbleBoPlingLoadViewModel.ChickenHomeScreenState.ChickenSuccess -> {
                        ...
                        ProBubbleBoPlingLoadViewModel.ChickenHomeScreenState.ChickenNotInternet -> {
